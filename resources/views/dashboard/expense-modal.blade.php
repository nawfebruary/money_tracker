                ...
                <div class="modal-body scroll-y m-5">
                    <form action={{ route('transaction.create') }} autocomplete="off">
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <!--begin::Select-->
                            <div id="expense_selector">
                                <input type="hidden"  name="is_custom" id="expense_is_custom"
                                value="" />
                                <input type="hidden"  name="category" id="expense_category"
                                value="" />
                                <button type="button" id="expense_a" name="category"
                                    class="form-control form-control-solid min-45px text-start min-h-45px"><span class="opacity-50">Choose category</span></button>
                                    <input type="text" id="expense_me"
                                        class="form-control form-control-solid bg-transparent d-none p-3 border border-1 rounded-top"
                                        placeholder="Add new category" oninput="validateSpace(this)" />

                                        @if(count($categories))
                                            <ul id="expense_mee" class="d-none  border-1 rounded-bottom border-top-0">
                                                @foreach ($categories->groupBy('type')['expense'] as $category)
                                                    <li class="list-group-item g-hover-secondary border-0 border-bottom-dotted" name="{{ $category->id }}">
                                                        {{ $category->name }}</li>
                                                @endforeach
                                            </ul>
                                        @endif
                        ...
                        <div class="fv-row mb-8">
                            ...
                            <!--end::Label-->
                            <!--begin::Input-->
                            <input type="number" class="form-control form-control-solid min-h-45px fs-6 rounded"
                                placeholder="Enter Amount" name="amount" />
                            <!--end::Input-->

                            <!--begin::Input-->
                            <input type="hidden" class="form-control form-control-solid" name="isIncome"
                                value="0" />
                            <!--end::Input-->
                        </div>
                        <!--end::Input group-->
